#!/usr/bin/env bb
;; 撓 tawami — flexibility-ledger kotoba Datom-log writer (content-addressed).
(ns tawami.methods.kotoba
  "kotoba.cljc — 撓 tawami append-only FLEXIBILITY LEDGER writer (on ADR-2605262130
  + ADR-2605312345; same content-addressed commit-DAG machinery as the mio/busshi
  family).

  Persists the flexibility OBSERVATIONS to a local append-only EDN log of
  content-addressed transactions. Each tx CID = 'b' + sha256-hex over the canonical
  JSON {\"datoms\":[…],\"prev\":…}. prev-cid chaining makes the log tamper-evident
  ...
  appends to a local file. A flexibility map, never a dispatch order."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; 撓 tawami — FLEXIBILITY LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of flexibility observations). Generated; DO NOT hand-edit. "
                        "A flexibility map, never a dispatch order.\n")))
